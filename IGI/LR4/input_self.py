...
def check_string(data):
    """
        ...
        date: 20.04.2025
        function is developed for analyzing user input for its acceptance for string
        """
    while not(all(char not in string.digits for char in data) and all(char not in string.punctuation for char in data) and all(char not in string.punctuation for char in data) and all(char not in string.whitespace for char in data)):
        data = input(("Введите строку еще раз, она не является словом с точки зрения языка! "))
    return data
